...
- `import_notes` can also instruct splitting one scraped row into multiple DB listings — e.g. "Split into two listings" followed by per-venue details (address, Maps link, day range) for each. Insert one row per venue named distinctly (e.g. append the venue name), each with its own resolved lat/lng — don't merge them back into one row with combined days_times.
